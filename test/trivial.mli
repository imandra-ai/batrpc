...
(** Beancount service *)
module Beancount : sig
  open Pbrt_services
  open Pbrt_services.Value_mode
  
  module Client : sig
    
    val add_stream : (single_int, stream, count, unary) Client.rpc
    
    val get_smaller_ints : (count, unary, single_int, stream) Client.rpc
  end
  
  module Server : sig
    (** Produce a server implementation from handlers *)
    val make : 
      add_stream:((single_int, stream, count, unary) Server.rpc -> 'handler) ->
      get_smaller_ints:((count, unary, single_int, stream) Server.rpc -> 'handler) ->
      unit -> 'handler Pbrt_services.Server.t
  end
end
