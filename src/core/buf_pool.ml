type t = {
  enc: Pbrt.Encoder.t Rpool.t;
  bufs: bytes Rpool.t;  (** Buffer of at least 16kB *)
  buf_size: int;
}
...
let create ?(enc_size = default_enc_size_) ?(buf_size = default_buf_size_) () :
    t =
  let enc =
    Rpool.create ~clear:Pbrt.Encoder.reset
      ~mk_item:(fun () -> Pbrt.Encoder.create ~size:enc_size ())
      ~max_size:24 ()
  and bufs =
    Rpool.create ~mk_item:(fun () -> Bytes.create buf_size) ~max_size:24 ()
  in
  { enc; bufs; buf_size }

let[@inline] with_enc self f = Rpool.with_resource self.enc f
...
let with_buf (self : t) size f =
  if size > self.buf_size then
    ...
    Rpool.with_resource self.bufs f
